 ...
 * Preview stores rows in import_batches (counts/errors JSONB) without
 * passwords; the commit step re-checks batch state and file hash, then
 * upserts voters atomically — re-import never duplicates accounts, never
 * overwrites an existing password or admin-adjusted rights (P4-04).
 * Optional per-row initial password is hashed for delivery to the committee
 * in the SAME response as the commit result and never persisted (P4-05).
 */

const MAX_ROWS = 2_000
const REQUIRED_COLUMNS = ['VOTER_TYPE', 'IDENTIFIER_TYPE', 'IDENTIFIER_VALUE', 'NAME', 'DEPARTMENT_CODE'] as const

type ParsedRow = { row: number; input: VoterUpsert }
...
export async function buildVoterTemplateXlsx(): Promise<Buffer> {
  ...
  ws.addRow(['STUDENT', 'NIM', '21101152610086', 'CONTOH — Nama Mahasiswa', 'KEP']).commit()
  ws.addRow(['LECTURER', 'NIP_LOCAL', '197512342005011005', 'CONTOH — Nama Dosen', 'KEB']).commit()
  ...
  notes.addRow(['DEPARTMENT_CODE', 'Kode jurusan sesuai sistem (mis. IF, SI); wajib sudah terdaftar']).commit()
...
export async function parseVoterWorkbook(file: Buffer): Promise<{ rows: ParsedRow[]; errors: RowError[] }> {
  ...
  dataSheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
    ...
    const zod = voterUpsertSchema.safeParse(input)
    if (!zod.success) {
 ...
 * Password-bearing cells are NOT accepted here by design (P4-05) — initial
 * passwords come from the admin import form to the committee only.
 */
 ...
 * never changes password_hash or admin-adjusted rights, and re-verifies the
 * election is still DRAFT before writing.
 */
  ...
  try {
    ...
    for (const pr of parsed_) {
      const res = await client.query<{ action: string }>(
        ...
        // Note: never touches password_hash here (P4-04).
      ...
      else if (res.rows[0].action === 'UPDATED') counts.updated++
      else counts.added++
    }
    await client.query(
...
/** Optional committee-issued initial passwords (P4-05): generate + return once. */
export async function issueInitialPasswords(actorId: string, identifierValues: string[]) {
  const pool = getPool()
  const out: { identifierValue: string; password: string }[] = []
  const client = await pool.connect()
  try {
    await client.query('BEGIN')
    for (const idv of identifierValues) {
      const { rows } = await client.query<{ id: string }>(
        `SELECT u.id FROM users u JOIN voters v ON v.id = u.voter_id WHERE v.identifier_value = $1 FOR UPDATE`, [idv],
      )
      if (!rows[0]) apiError('VALIDATION_ERROR', `Pemilih ${idv} belum memiliki akun.`)
      const password = generatePassword()
      const hash = await hashPassword(password)
      await client.query('UPDATE users SET password_hash = $2, credential_version = credential_version + 1 WHERE id = $1', [rows[0].id, hash])
      await recordAuditEvent(client, { actorId, action: 'ADMIN_ISSUE_INITIAL_PASSWORD', target: idv })
      out.push({ identifierValue: idv, password })
    }
    await client.query('COMMIT')
  } catch (err) {
    await client.query('ROLLBACK')
  ...
  return out // shown once to the committee; never logged or persisted
}
