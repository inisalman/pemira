...
type OptionResult = { optionId: string; number: number; label: string; votes: number; percent: number }
      ...
      <article v-for="contest in results" :key="contest.contestId" :class="$style.contest">
        ...
        <ol :class="$style.options" :aria-label="`Perolehan ${contest.title}`">
          <li v-for="option in contest.options" :key="option.optionId" :class="$style.option">
            ...
            <div :class="$style.bar" aria-hidden="true"><span :class="$style.fill" :style="{ width: `${Math.min(option.percent, 100)}%` }" /></div>
...
.loading, .empty { background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius-lg); padding: var(--space-6); }.empty h2 { font-size: var(--text-lg); }.empty p { color: var(--color-text-muted); margin-bottom: 0; }
.list { display: grid; gap: var(--space-4); }.contest { background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius-lg); padding: var(--space-4); }.contestHeader { display: flex; justify-content: space-between; gap: var(--space-4); padding-bottom: var(--space-4); border-bottom: 1px solid var(--color-border); }.contestHeader h2 { font-size: var(--text-lg); margin-bottom: var(--space-1); }.contestHeader p, .percent { color: var(--color-text-muted); font-size: var(--text-sm); }.contestHeader p { margin: 0; }.contestHeader strong { color: var(--color-primary-strong); white-space: nowrap; }
.options { display: grid; gap: var(--space-4); list-style: none; padding: 0; margin: var(--space-4) 0 0; }.option { display: grid; gap: var(--space-2); }.optionTopline { display: grid; grid-template-columns: auto minmax(0, 1fr) auto; gap: var(--space-2); align-items: center; }.number { align-items: center; background: var(--color-primary-soft); border-radius: var(--radius); color: var(--color-primary-strong); display: inline-flex; font-weight: 700; height: 2rem; justify-content: center; min-width: 2rem; }.label { font-weight: 600; min-width: 0; overflow-wrap: anywhere; }.bar { background: #d9e4df; border-radius: 0.25rem; height: 0.75rem; overflow: hidden; }.fill { background: var(--color-primary); display: block; height: 100%; }
@media (max-width: 42rem) { .page { gap: var(--space-4); }.header, .contestHeader { align-items: start; flex-direction: column; }.meta { text-align: left; }.status { justify-self: start; }.contest { padding: var(--space-3); }.contestHeader strong { white-space: normal; } }
</style>
